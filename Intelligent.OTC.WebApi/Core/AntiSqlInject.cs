...
namespace Intelligent.OTC.WebApi.Core
{
    ...
    public class AntiSqlInject
    {
        ...
        static AntiSqlInject()
        {
            ...
            SqlKeywordsArray.AddRange(Array.ConvertAll(SqlCommandKeywords.Split('|'), h => " " + h));
            SqlKeywordsArray.AddRange(Array.ConvertAll(HtmlCommandKeywords.Split('|'), h => h));
        }
        ...
        private const string SqlCommandKeywords = "and|exec|execute|insert|select|delete|update|count|chr|mid|master|" +
                                                  ...
                                                  "information_schema.columns|table_schema|union|where|select|delete|update|orderhaving|having|by|count|*|truncate|like|sleep|waitfor|delay|hyperlink";

        //private const string SqlSeparatKeywords = "'||--|\'|\"|/*|%|#";
        private const string HtmlCommandKeywords = "<cript>|</script>|alert(";
        ...
        public string GetSafetySql(string input)
        {
            ...
            foreach (var sqlKeyword in SqlKeywordsArray)
            {
                if (input.ToLower().IndexOf(sqlKeyword, StringComparison.Ordinal) >= 0)
                {
