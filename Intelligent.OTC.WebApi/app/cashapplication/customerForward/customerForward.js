angular.module('app.cashapplication.customerForward', [])
    .config(['$routeProvider', function ($routeProvider) {
        $routeProvider
            .when('/ca/forward', {
                templateUrl: 'app/cashapplication/customerForward/customerForward-list.tpl.html',
                controller: 'customerForwardListCtrl'
            });
    }])

    //*****************************************header***************************s
    .controller('customerForwardListCtrl',
        ['$scope', '$interval', 'customerForwardProxy', 'modalService',
            function ($scope, $interval, customerForwardProxy, modalService) {
                ...
                $scope.dataList = {
                    ...
                    columnDefs: [
                        ...
                        { field: 'forwardGroup', displayName: 'ForwardGroup', width: '20%' },
                        { field: 'forwardNum', displayName: 'ForwardNum', width: '10%' },
                        { field: 'forwardName', displayName: 'ForwardName', width: '20%' },
                        { field: 'customerNum', displayName: 'CustomerNum', width: '10%' },
                        { field: 'customerName', displayName: 'CustomerName', width: '20%' },
                        { field: 'localizeCustomerName', displayName: 'LocalizeCustomerName', width: '20%' }
                    ],
                ...
                $scope.pageSizeChange = function (selectedLevelId) {
                    customerForwardProxy.getForwarder($scope.currentPage, selectedLevelId, $scope.legalEntity ,$scope.customerNum,$scope.forwardNum ,$scope.forwardName , function (result) {
                ...
                $scope.pageChanged = function () {
                    customerForwardProxy.getForwarder($scope.currentPage, $scope.itemsperpage, $scope.legalEntity, $scope.customerNum, $scope.forwardNum, $scope.forwardName ,function (result) {
                        $scope.totalItems = result.listCount;
                        $scope.dataList.data = result.list;
                        $scope.startIndex = ($scope.currentPage - 1) * $scope.itemsperpage;
                ...
                $scope.editModal = function (row) {
                    ...
                    var modalDefaults = {
                        templateUrl: 'app/cashapplication/customerForward/customerForward-edit.tpl.html?2',
                        controller: 'customerForwardEditCtrl',
                        size: 'lg',
                        resolve: {
                            cont: function () {
                                return row;
                ...
                $scope.delete = function (id) {
                    ...
                    modalService.showModal(modalDefaults, {}).then(function (result) {
                        if (result == "Yes") {
                            ...
                            customerForwardProxy.deleteForwarder(id, function () {
                                $scope.pageChanged();
                            }, function (error) {
                                alert(error);
                            })
                ...
                $scope.export = function () {
                    ...
                    customerForwardProxy.export();
                };
        }])

    .controller('customerForwardEditCtrl', ['$scope', '$uibModalInstance', 'cont', 'customerForwardProxy','custAndBankCustProxy',
        function ($scope, $uibModalInstance, cont, customerForwardProxy, custAndBankCustProxy) {
            ...
            $scope.save = function () {
                ...
                if ( !$scope.cont.customerNum || !$scope.cont.customerName || !$scope.cont.forwardNum || !$scope.cont.forwardName) {
                    alert("Input canot be null");
                    return;
                }

                customerForwardProxy.updateForwarder($scope.cont,
                    function () {
                        $uibModalInstance.close();
                    },
                    function (res) {
