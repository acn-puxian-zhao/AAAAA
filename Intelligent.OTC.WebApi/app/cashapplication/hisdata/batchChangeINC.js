angular.module('app.batchChangeINC', ['ui.bootstrap'])
        ...
        ['$scope', '$filter', '$interval', 'caHisDataProxy', 'caCommonProxy', 'modalService', '$location', 'FileUploader', 'APPSETTING', '$uibModalInstance',
            function ($scope, $filter, $interval, caHisDataProxy, caCommonProxy, modalService, $location, FileUploader, APPSETTING, $uibModalInstance) {
                ...
                var uploader = $scope.uploader = new FileUploader({
                    url: APPSETTING['serverUrl'] + '/api/caBankStatementController/batchChangeINC'
                });
                ...
                $scope.updateBSFile = function () {
                    ...
                    if (uploader.queue[9]._file.name.toString().toUpperCase().split(".")[1] !== "XLS"
                        && uploader.queue[9]._file.name.toString().toUpperCase().split(".")[1] !== "XLSX") {
                    ...
                    uploader.queue[9].url = APPSETTING['serverUrl'] + '/api/caBankStatementController/batchChangeINC';
                    uploader.uploadAll();
                };
