angular.module('app.reportFeedbackByCs', [])
    .config(['$routeProvider', function ($routeProvider) {
        $routeProvider
            .when('/reportFeedbackByCs', {
                templateUrl: 'app/common/report/reportFeedbackByCs.tpl.html',
                controller: 'reportFeedbackByCsCtrl'
            });
    }])
        ...
        function ($scope, $interval, reportProxy, modalService) {
            ...
            $scope.reportFeedbackByCsList = {
                ...
                columnDefs: [
                    ...
                    { field: 'cs', displayName: 'CS', width: '200' },
            ...
            reportProxy.getFeedbackStatisticsByCs(function (result) {
                $scope.reportFeedbackByCsList.data = result;
            }, function (error) {
                alert(error);
            });
            ...
            $scope.pageSizeChanged = function (selectedLevelId) {
                reportProxy.getFeedbackDetailsByCs($scope.currentPage, selectedLevelId, function (result) {
            ...
            $scope.pageChanged = function () {
                reportProxy.getFeedbackDetailsByCs($scope.currentPage, $scope.itemsperpage, function (result) {
                    $scope.totalItems = result.count;
                    $scope.reportList.data = result.dataRows;
                    $scope.startIndex = ($scope.currentPage - 1) * $scope.itemsperpage;
            ...
            $scope.exportReport = function () {
                reportProxy.downloadFeedbackReportByCs(function (path) {
