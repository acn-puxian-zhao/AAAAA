angular.module('app.overduereport', [])
    .config(['$routeProvider', function ($routeProvider) {
        $routeProvider
            .when('/overduereport', {
                templateUrl: 'app/common/report/overduereport.tpl.html',

                controller: 'overduereportCtrl',
                resolve: {
                    //首次加载第一页
                    ...
                    statuslist: ['baseDataProxy', function (baseDataProxy) {
                        return baseDataProxy.SysTypeDetail("037");
                    }],
    ...
    //*****************************************header***************************s
    .controller('overduereportCtrl',
    ['$scope', '$interval', 'overdueReportProxy', 'modalService', 'overdueReasons','statuslist', 'legallist','ebList',
        function ($scope, $interval, overdueReportProxy, modalService, overdueReasons, statuslist, legallist, ebList) {
            $scope.$parent.helloAngular = "OTC - Overdue Report";
            $scope.legallist = legallist;
            $scope.overdueReasons = overdueReasons;
            $scope.ebList = ebList;
            $scope.statuslist = statuslist;
            $scope.overdueReason = "";

            var search = {};
            search.overdueReason = $scope.overdueReason;

            overdueReportProxy.queryReport(1, 20, angular.toJson(search), function (json) {
            ...
            $scope.reportList = {
                ...
                columnDefs: [
                    ...
                    { field: 'overdueReason', displayName: 'Due Reason', width: '150' },
                    ...
                    { field: 'vatDate', displayName: 'VAT date', width: '80', cellFilter: 'date:\'yyyy-MM-dd\''},
                    ...
                    { field: 'isForwarder', displayName: 'IsForwarder', width: '110' },
                    { field: 'forwarder', displayName: 'Forwarder', width: '90' }
                ],
            ...
            $scope.searchReport = function () {
                ...
                $scope.currentPage = 1;
                overdueReportProxy.queryReport($scope.currentPage, $scope.itemsperpage, angular.toJson(search), function (json) {
                    if (json != null) {
                        $scope.totalItems = json.totalItems;
                        $scope.members = json.list;
                    }
            ...
            $scope.pagesizechange = function (selectedLevelId) {
                var search = {};
                search.legaleEntity = $scope.legaleEntity;
                search.custCode = $scope.custCode;
                search.custName = $scope.custName;
                search.invoicecode = $scope.invoicecode;
                search.duedate = $scope.duedate;
                ...
                overdueReportProxy.queryReport(index, selectedLevelId, angular.toJson(search), function (json) {
            ...
            $scope.pageChanged = function () {
                ...
                //alert("d");
                var index = $scope.currentPage;
                overdueReportProxy.queryReport(index, $scope.itemsperpage, angular.toJson(search), function (json) {
                    $scope.members = json.list;
                    $scope.totalItems = json.totalItems;
                }, function (error) {
                    alert(error);
            ...
            $scope.exportReport = function () {
                ...
                overdueReportProxy.downloadReport(angular.toJson(search), function (path) {
