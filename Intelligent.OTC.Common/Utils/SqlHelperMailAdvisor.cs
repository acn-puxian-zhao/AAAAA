...
{
    public static class SqlHelperMailAdvisor
    {
        /// <summary>
        /// 数据库连接字符串
        /// </summary>
        public static readonly string ConnectionString = string.Format(ConfigurationManager.ConnectionStrings["MailAdvisorInterface"].ToString(), AESUtil.AESDecrypt(ConfigurationManager.AppSettings["OTC_Password"].ToString()));
        ...
        public static void ExcuteSql(string sql, params SqlParameter[] ps)
        {
            using (SqlConnection conn = new SqlConnection(ConnectionString))
            {
                conn.Open();
        ...
        public static void ExcuteListSql(List<string> sql)
        {
            using (SqlConnection conn = new SqlConnection(ConnectionString))
            {
                conn.Open();
        ...
        public static void ExcuteListSql(List<string> sql, List<SqlParameter[]> commandParameters)
        {
            using (SqlConnection conn = new SqlConnection(ConnectionString))
            {
                conn.Open();
                ...
                foreach (string s in sql)
                {
                    PrepareCommand(cmd, conn, null, CommandType.Text, s, commandParameters[i]);
                    object obj = cmd.ExecuteScalar(); //标识列返回的值的类型不是int而是decimal
        ...
        public static object ExecuteScalar(string v, CommandType commandType, string commandText, params SqlParameter[] commandParameters)
        {
            SqlCommand cmd = new SqlCommand();
            cmd.CommandTimeout = 0;
        ...
        /// <returns></returns>
        static public DataSet ExecuteDataSet(string commandText, CommandType commandType, params SqlParameter[] commandParameters)
        {
            DataSet ds = new DataSet();
