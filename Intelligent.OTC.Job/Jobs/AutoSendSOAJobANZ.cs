...
namespace Intelligent.OTC.Job
{
    ...
    public class AutoSendSOAJobANZ : BaseJob
    {
        ...
        protected override void ExecuteInternal(IJobExecutionContext context)
        {
            try
            {
                ...
                //判断是否有缺少AR的LegalEntity(仅ANZ)
                List<string> legalEntityList = new List<string>();
                legalEntityList.Add("308,");
                legalEntityList.Add("309,");
                bool lb_flag = false;
                ...
                Helper.Log.Info("1111111111111111111111111111111111");

                //判断当前日期是否需要发送SOA
                if (checkNeedSoa(deal))
                {
                    Helper.Log.Info("22222222222222222222222222");
                    //Send
        ...
        private bool checkNeedSoa(string deal) {
            ...
            List<CollectorAlert> nAlertKeyList = CommonRep.GetQueryable<CollectorAlert>().Where(x => x.Status == "Initialized" && x.ToName != "" && x.Deal == deal && x.AlertType != 5 && x.TempleteLanguage == "011" ).ToList();
        ...
        {
            List<string> nLegalEntityList = CommonRep.GetQueryable<CollectorAlert>().Where(x => x.Status == "Initialized" && x.ToName != "" && x.Deal == deal && x.AlertType != 5 && x.AlertType != 0 && x.TempleteLanguage == "011").Select(o => o.LegalEntity).Distinct().ToList();
        ...
        {
            Helper.Log.Info("33333333333333333333333");
            int intCount = 0;
            int index = 0;
            ...
            //{
                ...
                #region 2-1: 011
                nAlertKeyToCustomerList = CommonRep.GetQueryable<CollectorAlert>().Where(x => x.Status == "Initialized" && x.ToName != "" && (x.GroupName == null ? "" : x.GroupName) == "" && x.Deal == deal && x.AlertType != 5 && x.TempleteLanguage == "011" && x.ToName != "").Select(x => new AlertKey() { Region = x.Region, Deal = x.Deal, Collector = x.Eid, AlertType = x.AlertType, PeriodId = x.PeriodId, ToTitle = x.ToTitle, ToName = x.ToName, CCTitle = x.CCTitle, ResponseDate = x.ResponseDate, CustomerNum = x.CustomerNum, TempleteLanguage = x.TempleteLanguage }).Distinct()
                    .OrderBy(o => o.Region).ThenBy(o => o.ToTitle).ThenBy(o => o.ToName).ToList();
        ...
        {
            //SOA发送结果Mail
            List<SendSoaRemindingSum> soaAlertSum = soaService.GetSoaSendWarningSumANZ();
            List<SendSoaRemindingDetail> soaAlertDetail = soaService.GetSoaSendWarningDetailANZ();
