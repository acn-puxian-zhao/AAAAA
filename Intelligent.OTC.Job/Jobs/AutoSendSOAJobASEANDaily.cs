...
namespace Intelligent.OTC.Job
{
    ...
    public class AutoSendSOAJobASEANDaily : BaseJob
    {
        ...
        protected override void ExecuteInternal(IJobExecutionContext context)
        {
            try
            {
                ...
                //判断是否有缺少AR的LegalEntity(仅ASEAN)
                List<string> legalEntityList = new List<string>();
                legalEntityList.Add("302,");
                legalEntityList.Add("306,");
                legalEntityList.Add("4080,");
                bool lb_flag = false;
        ...
        private bool checkNeedSoa(string deal) {
            ...
            List<CollectorAlert> nAlertKeyList = CommonRep.GetQueryable<CollectorAlert>().Where(x => x.Status == "Initialized" && x.ToName != "" && x.Deal == deal && x.AlertType == 0 && (x.TempleteLanguage == "009" || x.TempleteLanguage == "006")).ToList();
        ...
        {
            List<string> nLegalEntityList = CommonRep.GetQueryable<CollectorAlert>().Where(x => x.Status == "Initialized" && x.ToName != "" && x.Deal == deal && x.AlertType == 0 && (x.TempleteLanguage == "009" || x.TempleteLanguage == "006")).Select(o => o.LegalEntity).Distinct().ToList();
        ...
        private int autoSendPTP(string deal,string endTime)
        {
            ...
            if (nAlertKeyToCustomerList != null)
            {
                foreach (AlertKey group in nAlertKeyToCustomerList)
                {
                    ...
                    logger.Info("---------------------strCollector---------------------" + group.Collector);
                    logger.Info("---------------------strdeal---------------------" + deal);
                    logger.Info("---------------------strLegalEntity---------------------" + group.LegalEntity);
                    logger.Info("---------------------alertCustomerNum---------------------" + group.CustomerNum);
                    logger.Info("---------------------alertSiteUseId---------------------" + group.SiteUseId);
                    logger.Info("---------------------alertType---------------------" + group.AlertType.ToString());
                    logger.Info("---------------------strToTitle---------------------" + group.ToTitle);
                    logger.Info("---------------------strToName---------------------" + group.ToName);
                    logger.Info("---------------------strGroupName---------------------" + group.GroupName);
                    logger.Info("---------------------strTempleteLanguage---------------------" + group.TempleteLanguage);
                    ...
                    logger.Info("---------------------CustomerNum---------------------" + group.CustomerNum);
                }
            }
            for (int i = 0; i < nAlertKeyToCustomerList.Count; i++)
            {
                ...
                Thread.Sleep(5000);    //30秒1个Mail
            }
            intCount += nAlertKeyToCustomerList.Count();
            #endregion

            #region 2-2: 006
            nAlertKeyToCustomerList = CommonRep.GetQueryable<CollectorAlert>().Where(x => x.Status == "Initialized" && x.ToName != "" && (x.GroupName == null ? "" : x.GroupName) == "" && x.Deal == deal && x.AlertType == 0 && (x.TempleteLanguage == "009"|| x.TempleteLanguage == "006") && x.ToName != "").Select(x => new AlertKey() { Region = x.Region, Deal = x.Deal, Collector = x.Eid, AlertType = x.AlertType, PeriodId = x.PeriodId, ToTitle = x.ToTitle, ToName = x.ToName, CCTitle = x.CCTitle, ResponseDate = x.ResponseDate, CustomerNum = x.CustomerNum, TempleteLanguage = x.TempleteLanguage }).Distinct()
                .OrderBy(o => o.Region).ThenBy(o => o.ToTitle).ThenBy(o => o.ToName).ToList();
        ...
        private void autoRemindSOAError(string deal, string to, List<string> legalEntityList)
        {
            ...
            sb.Append("</p>");
            MailTmp mail = new MailTmp();
            mail.Subject = "***** IOTC - SOA Send Error（Daily) *****";
            mail.Body = sb.ToString();
            mail.From = senderMailbox;
            mail.To = to;
            mail.Deal = deal;
