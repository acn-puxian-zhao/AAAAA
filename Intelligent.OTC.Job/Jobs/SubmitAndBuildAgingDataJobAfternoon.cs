...
namespace Intelligent.OTC.Job
{
    ...
    public class SubmitAndBuildAgingDataJobAfternoon : BaseJob
    {
        ...
        protected override void ExecuteInternal(IJobExecutionContext context)
        {
            try
            {
                ...
                string uploadResult = CheckFileUploadStatus1();
                ...
                try
                {
                    ...
                    if (submitResult == 0)
                    {
                        logger.Info(string.Format("SubmitAndBuildAgingDataJobAternoon-Aging failed,DEAL:{0}", batchDeal));
                    }
                }
                catch (Exception ex) {
            ...
            catch (Exception ex)
            {
                ...
                logger.Error("SubmitAndBuildAgingDataJobAfternoon,error!" + ex.Message, ex);
            }
        ...
        private void autoBuildContactor() {
            ...
            string strSQL = @"INSERT INTO dbo.T_CONTACTOR
                                ...
                                ISNULL((SELECT TOP 1 EMAIL_ADDRESS FROM T_CONTACTOR WHERE TITLE = 'CS' AND NAME = inv.LsrNameHist),(SELECT TOP 1 EMAIL_ADDRESS FROM T_CONTACTOR_HIS WHERE TITLE = 'CS' AND NAME = inv.LsrNameHist))AS EMAIL_ADDRESS, 
                                ...
                                ISNULL((SELECT TOP 1 EMAIL_ADDRESS FROM T_CONTACTOR WHERE TITLE = 'Sales' AND NAME = inv.FsrNameHist), (SELECT TOP 1 EMAIL_ADDRESS FROM T_CONTACTOR_HIS WHERE TITLE = 'Sales' AND NAME = inv.FsrNameHist)), 
        ...
        /// </summary>
        /// <param name="checkDate"></param>
        private string CheckFileUploadStatus1()
        {
            DateTime nNowDate = DateTime.Now;
            ...
            Helper.Log.Info("Start: call p_CheckFileUploadStatus1(procedure):@InputDate" + nNowDate.ToString());

            string result = "";
            DataTable nIdTable = CommonRep.ExecuteDataTable(CommandType.StoredProcedure, "p_CheckFileUploadStatus1", paramBuildList.ToArray());
            if (nIdTable != null && nIdTable.Rows.Count > 0)
            {
                result = nIdTable.Rows[0][0].ToString();
            }

            Helper.Log.Info("End: call p_CheckFileUploadStatus1(procedure):@InputDate" + nNowDate.ToString() + ",RETURN:" + result);
