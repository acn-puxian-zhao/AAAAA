...
namespace Intelligent.OTC.Job
{
    ...
    public class AutoSendSOAJob : BaseJob
    {
        ...
        protected override void ExecuteInternal(IJobExecutionContext context)
        {
            try
            {
                ...
                //Build Alert
                var sites = CommonRep.GetQueryable<Sites>().Where(o => o.Deal == deal && o.Description == "SAP").ToList();
                ...
                try
                {
                    int submitResult = ServiceCustomer.SubmitInitialAgingNew(deal);
                    if (submitResult == 0)
                    {
                        logger.Info(string.Format("SubmitAndBuildAgingDataJob-Aging failed,DEAL:{0}", deal));
                    }
                }
                catch (Exception ex)
                {
        ...
        private bool checkNeedSoa(string deal) {
            ...
            List<CollectorAlert> nAlertKeyList = CommonRep.GetQueryable<CollectorAlert>().Where(x => x.Status == "Initialized" && x.ToName != "" && x.Deal == deal && x.AlertType != 5 && x.TempleteLanguage != "006" && x.TempleteLanguage != "009" && x.TempleteLanguage != "007" && x.TempleteLanguage != "0071" && x.TempleteLanguage != "011").ToList();
        ...
        {
            List<string> nLegalEntityList = CommonRep.GetQueryable<CollectorAlert>().Where(x => x.Status == "Initialized" && x.ToName != "" && x.Deal == deal && x.AlertType != 5 && x.AlertType != 0 && x.TempleteLanguage != "006" && x.TempleteLanguage != "009" && x.TempleteLanguage != "007" && x.TempleteLanguage != "0071" && x.TempleteLanguage != "011").Select(o=>o.LegalEntity).Distinct().ToList();
