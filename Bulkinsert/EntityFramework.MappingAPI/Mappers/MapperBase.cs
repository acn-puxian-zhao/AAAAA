    ...
    internal abstract class MapperBase
    {
        ...
        public Dictionary<string, EntityType> TypeMappings
        {
            get
            {
                ...
                var tmp = MetadataWorkspace.GetItems(DataSpace.OCSpace);
                _typeMappings = new Dictionary<string, EntityType>();
                foreach (var x in tmp)
                {
                    var entType = (x.GetPrivateFieldValue("EdmItem") as EntityType);
                    if (entType != null)
                    {
                        _typeMappings.Add(x.ToString().Split(':')[0], entType);
                    }
                }
        //        _typeMappings = 
        //.Select(x => new { identity = x.ToString().Split(':'), edmItem = x.GetPrivateFieldValue("EdmItem") as EntityType })
